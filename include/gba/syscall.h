...
static inline void CpuSetChecked(const void *src, void *dest, u32 control)
{
    CpuSet(src, dest, control);
}
#define CpuSet(src, dest, control) CpuSetChecked((src), (dest), (control))
#endif
...
#if MODERN && (__STDC_VERSION__ >= 201112L)
static inline void CpuFastSetChecked(const void *src, void *dest, u32 control)
{
    CpuFastSet(src, dest, control);
}
#define CpuFastSet(src, dest, control) CpuFastSetChecked((src), (dest), (control))
#endif
